...
class HTML {
	...
	public static function blog_url($username, $uri) {
		$url = UOJConfig::$data['wordpress']['address'] . "/author/" . $username . $uri;
		$url = rtrim($url, '/');
	...
	public static function js_src($uri) {
		return '<script src="'.UOJConfig::$data['cdn_prefix'].''.$uri.'"></script>';
	}
	public static function css_link($uri) {
		return '<link type="text/css" rel="stylesheet" href="'.UOJConfig::$data['cdn_prefix'].''.$uri.'" />';
	}
