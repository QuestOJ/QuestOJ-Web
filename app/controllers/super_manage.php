<?php
	...
	$group_form->addInput('groupname', 'text', '组别名称', '', 
		function ($groupname){
			if(empty($groupname)){
				return '组别名称不能为空';
			}else if(!validateString($groupname)){
				return '不能含有关键字';
			}else if($groupname == "未分类"){
				return '系统内置用户组';
			}
			return '';
		},
	...
	$group_form->addSelect('op-type', $options, '操作类型', '');
		...
		switch ($_POST['op-type']){
			case 'addgroup':
				if(!queryGroup($groupname))
					DB::insert("insert into `usergroup` (`name`) VALUES ('{$groupname}')");
				break;
			case 'delgroup':
				echo "<script>alert({$groupname});</script>";
				DB::delete("delete from `usergroup` where name = '{$groupname}'");
				break;
		}
	};
	...
	$group_cols = array('id', 'name');
	$group_config = array();
	$group_header_row = <<<EOD
	<tr>
		<th>ID</th>
		<th>组名</th>
	</tr>
EOD;
	$group_print_row = function($row) {
		$gid = $row['id'];
		$gname = $row['name'];
		echo <<<EOD
			<tr>
...
EOD;
	...
	$list_form->addInput('username', 'text', '用户名', '',
		function ($username) {
	...
	$options = array();
	$grouplist = DB::SelectAll("SELECT * FROM `usergroup`");
	foreach($grouplist as $group){
		$options[$group['id']] = $group['name'];
	}
	$list_form->addSelect('gid', $options, '用户组', '');
	...
	$list_form->handle = function(){
		...
		DB::update("update user_info set userdefine = '{$gid}' where username = '{$username}'");
	};
...
<div class="row">
	...
	<div class="col-sm-9">
		...
		<?php elseif ($cur_tab === 'group'): ?>
			...
			<div>
				<h3>用户组列表</h3>
				<?php echoLongTable($group_cols, 'usergroup', "id <> 0", '', $group_header_row, $group_print_row, $group_config) ?>
			</div>
		<?php elseif ($cur_tab === 'userlist'): ?>
			<h3>组别修改</h3>
			<?php $list_form->printHTML(); ?>
			<br/>
			<?php
				$all = DB::selectAll("SELECT * FROM `usergroup`");
				foreach ($all as $grouplist) {
					$cnt = 0;
					echo "<h4>{$grouplist['name']}</h4>";
					$user_all = DB::selectAll("SELECT * FROM `user_info` where userdefine = '{$grouplist['id']}' ");
					...
					foreach ($user_all as $user){
						...
						$cnt = $cnt + 1;
						$hislink = getUserLink($user['username']);
						echo "<td width = '150px'>${hislink}</td>";
