<?php
	...
	class SyncProblemDataHandler {
		...
		private function compile_at_prepare($name, $config = array()) {
			...
			$cmd_prefix = "$uojMainJudgerWorkPath/run/run_program >{$this->prepare_dir}/run_compiler_result.txt --in=/dev/null --out=stderr --err={$this->prepare_dir}/compiler_result.txt --tl=10 --ml=512 --ol=64 --unsafe --type=compiler --work-path={$work_path}";
		...
		private function makefile_at_prepare() {
			...
			$include_path = "$uojMainJudgerWorkPath/include";
			$cmd_prefix = "$uojMainJudgerWorkPath/run/run_program >{$this->prepare_dir}/run_makefile_result.txt --in=/dev/null --out=stderr --err={$this->prepare_dir}/makefile_result.txt --tl=10 --ml=512 --ol=64  --unsafe --type=compiler --work-path={$this->prepare_dir}";
